  ...
  return (
    <Shell language={language}>
      <main className="mx-auto max-w-3xl px-6 py-16">
        ...
        <div className="mt-10 space-y-8">
          {playbook.items.map((item, index) => (
            <article key={item.title} className="border-t border-line pt-6">
              ...
              <p className="mt-2 leading-relaxed text-muted">{item.why}</p>
        ...
        <section className="mt-14 rounded-xl border border-caution/30 bg-caution/5 p-6">
                ...
                <ul className="mt-2.5 space-y-1.5">
                  {item.steps.map((step) => (
                    <li key={step} className="flex gap-2.5 text-sm text-muted">
                      <span aria-hidden className="mt-2 inline-block size-1 shrink-0 rounded-full bg-caution" />
                      <span>{step}</span>
                    </li>
                  ))}
                </ul>
